// Script to insert 100 unique Tamil payers for the wedding function with denomination support for visaganorg tenant
...
interface Payer {
  ...
  net_amount?: number;
  // Add organization field for multi-tenant
  org_name: string;
}

// The organization we're creating data for
const ORG_NAME = 'visaganorg';

// The specific function_id we're adding payers for
const FUNCTION_ID = 'திருமண_விழா-ராஜேந்திரன்_சரவணன்-கோயம்புத்தூர்-2025-07-25-11:00_am';
...
const tamilRelations = [
  ...
  "தாய்மாமன்", "தாய்மாமன்மார்", "தாய்மாமன்மார்கள்"
];
...
const tamilCities = [
  ...
  // Cities with initials
  "A. பெருமாள்பட்டி", "G. உசிலம்பட்டி", "K. புதுப்பட்டி", "S. வாடிப்பட்டி", "T. கள்ளிப்பட்டி", "M. புலியம்பட்டி", "N. புளியம்பட்டி",
  "E. கோட்டைப்பட்டி", "P. வேம்பத்தூர்", "V. மணியம்பட்டி", "R. புதுப்பட்டி", "C. பழனிப்பட்டி", "KG. பட்டி", "KK. புரம்",
  "PC. பட்டி", "KM. பட்டி", "RMTC. காலனி", "J. சேர்வைப்பட்டி", "L. மதுரை", "B. மேட்டுப்பட்டி",
  
  // Additional cities
  "போடிநாயக்கனூர்", "பொள்ளாச்சி", "மயிலாடுதுறை", "குடியாத்தம்", "ஆரணி", "பழனி", "சிதம்பரம்", "பட்டுக்கோட்டை", "திருத்தணி", "ஓசூர்",
  "கிருஷ்ணகிரி", "தர்மபுரி", "ஆத்தூர்", "மேட்டூர்", "அரியலூர்", "ஜெயங்கொண்டம்", "திருவாரூர்", "காரைக்குடி", "திருப்பத்தூர்", "மன்னார்குடி",
  "வந்தவாசி", "திருப்பரங்குன்றம்", "சாத்தூர்", "தேனி", "பேரையூர்", "சங்கரன்கோவில்", "கொடைக்கானல்", "வாலாஜாபாத்", "ராணிப்பேட்டை", "ஆற்காடு",
  "இராஜபாளையம்", "சிவகங்கை", "மணப்பாறை", "கும்பகோணம்", "முத்துபேட்டை", "அரக்கோணம்", "வேப்பம்பட்டு", "செங்கல்பட்டு", "பாபநாசம்", "அம்மாபேட்டை",
  "அனுமந்தன்பட்டி", "அண்ணாநகர்", "அப்பிபட்டி", "அருப்புக்கோட்டை", "அலங்காநல்லூர்", "ஆனைமலைநகர்", "ஆண்டிபட்டி", "ஆத்திப்பட்டி", "அம்மாபட்டி", "அழகாபுரி",
  "அம்பூர்", "அய்யம்பாளையம்", "பாப்பாரப்பட்டி", "பள்ளிபாளையம்", "பட்டாணி", "போடிக்குளம்", "பேரூர்", "புலியூர்", "ராசிபுரம்", "ராஜபாளையம்",
  "சாத்தனூர்", "செய்யூர்", "சிதம்பரம்", "சின்னமனூர்", "சின்னசேலம்", "சூலூர்", "தம்பம்பட்டி", "தாராபுரம்", "தேவகோட்டை", "தேவதானப்பட்டி",
  "தாளவாடி", "தாம்பரம்", "தொப்பம்பட்டி", "தொட்டியம்", "துறையூர்", "உத்தமபாளையம்", "உடுமலை", "வடகரை", "வரகூர்", "விராலிமலை",
  "விளாத்திகுளம்", "வீரபாண்டி", "வெம்பக்கோட்டை", "வெள்ளையம்மாள்புரம்", "வேப்பமரத்துப்பட்டி", "வேலூர்"
];
...
// Create Function
async function createFunction(client: MongoClient, adminUserId: ObjectId) {
  const db = client.db(dbName);
  const functionsCollection = db.collection(`${ORG_NAME}_functions`);
  
  // Function details
  const functionDate = new Date("2025-07-25T11:00:00.000Z");
  const now = new Date();
  ...
  const functionDoc = {
    ...
    function_time: "11:00 AM",
    function_place: "கோயம்புத்தூர்",
    function_details: "ராஜேந்திரன் மற்றும் சரவணன் குடும்பத்தில் நடைபெற உள்ள திருமண நிகழ்வு",
    function_venue: "ஸ்ரீ சண்முக கல்யாண மண்டபம், கோயம்புத்தூர்",
    function_venue_address: "45, டி.வி.சாமி ரோடு, ஆர்.எஸ்.புரம், கோயம்புத்தூர் - 641002",
    function_host: "ராஜேந்திரன் குடும்பத்தினர்",
    function_type: "திருமணம்",
    status: "Upcoming",
    is_deleted: false,
  ...
  try {
    const result = await functionsCollection.insertOne(functionDoc);
    console.log(`Created function with ID: ${result.insertedId}`);
    return functionDoc;
  } catch (error) {
...
async function insertHundredPayers() {
  try {
    ...
    // Define collection - use organization-specific collections
    const payersCollection = db.collection(`${ORG_NAME}_payers`);
    ...
    if (!adminUserId) {
      throw new Error("Admin user not found! Please check if the admin user exists.");
    }
    
    // // Create function first
    // const functionDoc = await createFunction(client, adminUserId);
    // console.log(`Created function: ${functionDoc.function_name} for organization: ${ORG_NAME}`);
    ...
    const baseDate = new Date("2025-07-20T09:00:00.000Z");
    ...
    for (let i = 0; i < 100; i++) {
      ...
      const payer: Payer = {
        ...
        org_name: ORG_NAME // Add organization name for multi-tenant
      };
    ...
    console.log(`${payerResult.insertedCount} payers inserted for function: ${FUNCTION_ID} in organization: ${ORG_NAME}`);
